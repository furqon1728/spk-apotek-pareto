...
<body>
      ...
      <!-- Figure JUDUL TABEL -->
        <figure class="text-light">
          <blockquote class="blockquote">
            <h1>Tabel Data Obat</h1>
          </blockquote>
        </figure>
       <div class="row justify-content-between">
         <!-- BUTTON TAMBAH DATA OBAT -->
          <div class="col-4">
            <a href="kelola-obat.php" class="btn btn-primary active" role="button" aria-pressed="true">
  ...
  <!-- FEEDBACK PILIHAN KATEGORI DAN PENCARIAN -->
  <?php if (!empty($kategori) || !empty($cari)) { ?>
    <div class="mt-3 text-light">
      <?php if (!empty($kategori)) : ?>
        <p>Kategori dipilih: <strong><?= $kategori ?></strong></p>
        <a href="tabel-obat.php" class="btn btn-secondary">Reset Filter</a>
      <?php endif; ?>
      <?php if (!empty($cari)) : ?>
      ...
      <tbody class="table-group-divider">
        ...
        <tr>
          ...
          <td>
            ...
            <a type="button" class="btn btn-danger" href="proses-obat.php?hapus=<?php echo $result['kode_obat'];?>" onClick="return confirm('apakah anda yakin ingin menghapus data tersebut??')"> <i class="bi bi-trash"></i> </a>
          </td>
        </tr>
        <?php
        }
